...
pulse_input U = pulse_input(0.1, 0.5, 200);
int_constant_block zero = int_constant_block(0);
pulse_input U2 = pulse_input(1, 1.5, -200);
...
int main (int argc, char **argv)
{
  ...
  //bdsyssetupcode
  G_cart.set_input_blocks(&add, &zero);
  add.set_input_blocks(&U, &U2);
  ...
  for (i=0;i<N;i++){
     ...
     //bdsysprintcode
     fprintf(fp, "%i,%0.2f,%i,%i,%i\n",i,t_ms,U.read_output(),U2.read_output(),add.read_output());
