...
prev_hold_block prev_hold = prev_hold_block();
...
int main (int argc, char **argv)
{
  ...
  //bdsyssetupcode
  G_cart.set_input_blocks(&Uconst, &Uconst);
  prev_hold.set_input_block1(&pend_enc);
  ...
  for (i=0;i<N;i++){
     ...
     Uconst.find_output();
     G_cart.find_output();
     prev_hold.find_output();
     G_cart.send_commands(i);



     //bdsysprintcode
     fprintf(fp, "%i,%0.2f,%i,%i\n",i,t_ms,prev_hold.read_output(),pend_enc.read_output());
